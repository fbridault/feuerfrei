	...
	m_rTexture(a_rTexture)
{
	...
	m_texCPoints =  new GLfloat[(NB_PARTICLES_MAX + m_nbFixedPoints) * (m_nbSkeletons + m_uorder - 1) * 2];
...
void INurbsFlame::initNurbs(GLUnurbsObj** nurbs)
{
...
void INurbsFlame::drawNurbs () const
{
...
void INurbsFlame::drawLineFlame () const
{
	if (! (m_shadingType & 1) )
	{
...
IFixedFlame::~IFixedFlame()
{
}

void IFixedFlame::drawCPointFlame () const
{
	if (! (m_shadingType & 1) )
	{
		glColor4f (1.0f, 1.0f, 1.0f, 1.0f);
...
void IRealFlame::computeVTexCoords()
{
	float vinc, vtmp;
	uint i;
