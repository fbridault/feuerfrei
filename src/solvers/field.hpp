...
public:
   ...
   */
  Field (const CPoint& position, float timeStep, float buoyancy);
  /** Destructeur */
  virtual ~Field ();
   ...
   * @return Position dans l'espace.
   */
  virtual void setPosition (const CPoint& position) { m_position = position; };
   ...
   */
  virtual void move(const CPoint& forces) { m_movingForces += forces; }
   ...
   */
  virtual void addTemporaryExternalForces(const CPoint& forces) { m_temporaryExternalForces = forces; }

  /** Ajoute de façon permanente des forces externes sur une des faces du solveur. Cette méthode
   * est utilisée principalement lorsque que du vent est appliqué sur une flamme.
   * @param forces Intensité de la force en (x,y,z).
   */
  virtual void addPermanentExternalForces(const CPoint& forces) { m_permanentExternalForces = forces; }
