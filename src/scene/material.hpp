...
public:
   ...
   * @param tex CPointeur optionel sur la texture.
   */
   ...
   * @return CPointeur sur la texture
   */
  const ITexture* getDiffuseTexture() const { return m_scene->getTexture(m_diffuseTexture); };
...
private:
  ...
  const Scene *m_scene; /** CPointeur vers la scène, utilisé pour récupérer les textures. */
