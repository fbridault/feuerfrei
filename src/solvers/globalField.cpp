...
#define ARGS *pTransform, n_x, n_y, n_z, dim, timeStep, buoyancy
#define ARGS_SLV ARGS, vorticityConfinement
#define ARGS_GC ARGS_SLV, omegaDiff, omegaProj, epsilon

#ifdef MULTITHREADS
GlobalField::GlobalField(const list <FieldThread *> &threads, CSpatialGraph& a_rGraph, char type, uint n,
#else
GlobalField::GlobalField(const vector <Field3D *> &fields, CSpatialGraph& a_rGraph, char type, uint n,
#endif
                         float timeStep, float vorticityConfinement, float omegaDiff, float omegaProj, float epsilon)
{
	...
	a_rGraph.GetBoundingBox(min,max);
	width = max - min;
	position = min;
	...
	{
		case GS_SOLVER :
			m_pField = new GSSolver3D(ARGS_SLV);
			break;
		case GCSSOR_SOLVER :
			m_pField = new GCSSORSolver3D(ARGS_GC);
			break;
		case HYBRID_SOLVER :
			m_pField = new HybridSolver3D(ARGS_GC);
			break;
		case LOD_HYBRID_SOLVER :
			m_pField = new LODSolver3D(ARGS_GC);
			break;
		case SIMPLE_FIELD :
			m_pField = new RealField3D(ARGS);
			break;
		case FAKE_FIELD :
			m_pField = new FakeField3D(*pTransform, dim, timeStep, buoyancy);
			break;
		case LOD_FIELD :
			m_pField = new LODField3D(ARGS_GC);
			break;
		default :
...
void GlobalField::shareForces()
{
	...
	{
		CTransform const& rThisTransform = m_pField->GetTransform();
		CTransform const& rTransform = (*solversIterator)->GetTransform();
		...
		pt = rTransform.GetLocalPosition() - rThisTransform.GetLocalPosition();
		...
		strength[0] = m_pField->getUVW(pt,dump);
		strength[1] = m_pField->getUVW(pt+CPoint(0.0f,ldim.y,0.0f),dump);
		strength[2] = m_pField->getUVW(pt+CPoint(0.0f,ldim.y,ldim.z),dump);
		strength[3] = m_pField->getUVW(pt+CPoint(0.0f,0.0f,ldim.z),dump);
		strength[4] = m_pField->getUVW(pt+CPoint(ldim.x,ldim.y,0.0f),dump);
		strength[5] = m_pField->getUVW(pt+CPoint(ldim.x,0.0f,0.0f),dump);
		strength[6] = m_pField->getUVW(pt+CPoint(ldim.x,ldim.y,ldim.z),dump);
		strength[7] = m_pField->getUVW(pt+CPoint(ldim.x,0.0f,ldim.z),dump);
