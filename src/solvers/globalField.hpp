...
public:
   ...
   */
  virtual void addPermanentExternalForces(CPoint& forces){ m_field->addPermanentExternalForces(forces); }

  /** Ajoute de façon temporaire (une itération) des forces externes sur une des faces du solveur. Cette méthode
   * est utilisée principalement lorsque que du vent est appliqué sur une flamme.
   * @param forces Intensité de la force en (x,y,z).
   */
  virtual void addTemporaryExternalForces(CPoint& forces) { m_field->addTemporaryExternalForces(forces); }
   ...
   */
  CPoint getPosition (void) { return m_field->getPosition(); };
